...
class TranswaveUNET_c(nn.Module):
    ...
    def forward(self, inputs):                             # 2x 3  x 128x128
        ...
        dwt1, s1 = self.encoder_blocks[0](s0)             # 2x 256 x 32x32
        dwt2, s2 = self.encoder_blocks[1](dwt1)           # 2x 512 x 16x16
        dwt3, s3  = self.encoder_blocks[2](dwt2)          # 2x 1024 x 8x8
        
        x,skip_connections = self.VIT(dwt3)
        x = rearrange(x, "b (x y) c -> b c x y", x=8, y=8)  # 2x 1024 x 8 x 8

        x1 = self.bottelneck(x)                              # 2x 512 x 8 x 8

        idwt1 = self.decoder_blocks[0](x1,     s2)        # 2x 256 x  16x16
        idwt2 = self.decoder_blocks[1](idwt1,  s1)        # 2x 128 x  32x32
        idwt3 = self.decoder_blocks[2](idwt2,  s0)        # 2x 64  x  64x64
